/**
 * split_kv_kernel_v4.cu — CUDA decode attention, split-KV two-pass variant
 * with software-pipelined K/V loads via cp.async (LDGSTS), v4: reduced
 * register pressure vs. the pipelined baseline.
 *
 ...
 *
 * Delta vs. split_kv_kernel_pipelined.cu: drops the per-thread
 * `parts[PAGE_SIZE]` and `exp_weights[PAGE_SIZE]` register arrays — each
 * value is only ever needed once, right after it's produced, so both loops
 * stream a single scalar through instead of buffering PAGE_SIZE floats per
 * thread. Same math, lower register footprint, higher occupancy.
 *
...
// ── Pass 1: partition kernel (pipelined, v4 register-reduced) ─────────────

template<int HEAD_DIM, int PAGE_SIZE>
__global__ void decode_attn_partition_kernel_v4(
    ...
    for (int p = 0, fetch = NUM_STAGES; p < my_num_pages; p++, fetch++) {
        ...
        // ── QK scores ───────────────────────────────────────────────────
        // CHANGED: removed `float parts[PAGE_SIZE]` register array. The warp
        // reduction result only needs to reach lane 0's sm_warp slot, so we
        // stream it out per-token instead of buffering all PAGE_SIZE floats
        // per thread. Kills the biggest contributor to register pressure.
        #pragma unroll
        for (int tok = 0; tok < PAGE_SIZE; tok++) {
            float k_val = __half2float(stage_k[stage][tok * HEAD_DIM + tid]);
            float part  = warp_reduce_sum(q_val * k_val);
            if (lane_id == 0) {
                sm_warp[warp_id * PAGE_SIZE + tok] = part;
            }
        }
        __syncthreads();
        ...
        // ── Fused exp / l_block / V accumulate ──────────────────────────
        // CHANGED: removed `float exp_weights[PAGE_SIZE]` register array.
        // The exp weight is used exactly twice per token (once into l_block,
        // once into acc_block), both scalar accumulators, so there is no
        // reason to persist it. Keep a single scalar `w` live per iteration.
        float l_block   = 0.0f;
        float acc_block = 0.0f;
        #pragma unroll
        for (int tok = 0; tok < PAGE_SIZE; tok++) {
            float w      = expf(sm_qk[tok] - m_block);
...
static void launch_partition_hd(
    ...
    auto kernel = decode_attn_partition_kernel_v4<HEAD_DIM, PAGE_SIZE>;
...
torch::Tensor decode_attention_split_kv_cuda_v4(
    torch::Tensor q,
...
#define PART_ARGS q_ptr, kv_ptr,                               \
    ...
    nvtxRangePushA("cuda_split_kv_partition_v4");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "CUDA decode attention — split-KV two-pass kernel (v4, pipelined + reduced register pressure)";
    m.def("decode_attention_split_kv",
          &decode_attention_split_kv_cuda_v4,
          "Split-KV decode attention: pipelined partition + reduction (v4)",
